...
class ModuleExecutor:
    ...
    def execute_module(self, 
        ...
        try:
            ...
            # 参数映射和预处理
            processed_parameters = self._process_parameters(module_id, parameters)
            
            # 处理数据上下文
            processed_data_context = self._process_data_context(data_context)
            
            # 执行模块
            result = instance.execute(processed_parameters, processed_data_context)
            
            # 检查执行结果
            if result.get('success', False):
                logger.info(f"模块执行成功: {module_id}")
            else:
                logger.error(f"模块执行失败: {module_id}, 错误: {result.get('error', '未知错误')}")
    ...
    def create_execution_plan(self, strategy: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, strat in enumerate(strategies):
                ...
                # 检查模块是否已加载，如果没有则尝试从配置加载
                if module_id not in self.loaded_modules:
                    # 尝试从配置文件加载模块
                    self._load_module_from_config_file(module_id)
                
                # 如果模块存在（已加载或刚加载），则添加到执行计划
                if module_id in self.loaded_modules or self._module_exists(module_id):
                    ...
                    logger.warning(f"模块 {module_id} 不存在或无法加载，跳过执行计划")
    ...
    def _load_module_from_config_file(self, module_id: str):
        """
        从配置文件加载指定模块
        ...
        try:
            # 加载模块配置文件
            config_path = self.modules_dir / "analysis_config.json"
            if config_path.exists():
                ...
                for module_config in modules_list:
                    if module_config.get('module_id') == module_id:
                        self.load_module_from_config(module_config)
                        logger.info(f"从配置文件动态加载模块: {module_id}")
                        return
                
                logger.warning(f"配置文件中未找到模块: {module_id}")
            else:
                logger.warning(f"模块配置文件不存在: {config_path}")
        except Exception as e:
            logger.error(f"从配置文件加载模块 {module_id} 失败: {e}")
    ...
    def _module_exists(self, module_id: str) -> bool:
        ...
        try:
            # 检查常见的模块文件位置
            module_file = self.modules_dir / f"{module_id}_module.py"
            return module_file.exists()
        except Exception:
            return False
    
    def _process_parameters(self, module_id: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """
        处理和映射模块参数
        ...
        # 针对data_describe模块的特殊处理
        if module_id == 'data_describe':
            # 如果有data_path但没有data_source，进行映射
            if 'data_path' in processed and 'data_source' not in processed:
                processed['data_source'] = processed['data_path']
                logger.info(f"参数映射: data_path -> data_source = {processed['data_source']}")
        
        return processed
    
    def _process_data_context(self, data_context: Dict[str, Any]) -> Dict[str, Any]:
        """
        处理数据上下文，确保格式正确
        ...
        Returns:
            Dict[str, Any]: 处理后的数据上下文
        """
        if not data_context:
            return {}
        
        processed_context = data_context.copy()
        
        # 如果data_context是database_info格式，需要转换
        if 'type' in data_context and 'path' in data_context:
            # 对于文件系统类型的数据源，不需要db_connector
            if data_context.get('type') == 'file_system':
                processed_context['db_connector'] = None
            else:
                # 对于其他数据库类型，这里可以扩展创建相应的连接器
                processed_context['db_connector'] = None
        ...
        for step in execution_plan:
            try:
                ...
                results.append({
                    ...
                    "output": result.get("summary", result.get("result", "")),
