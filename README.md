...
基于终端实际运行日志和代码分析，我已成功更新了 README_sales_query.md 文档，并总结了 graph_builder.py 的工作流和销量查询能力：
...
1. 1. 意图识别节点 ( recognize_intent_node )

   - 使用 GLM 分析用户查询意图
   - 支持 query_only （直接查询）、 data_analysis （数据分析）、 general_chat （一般对话）
   - 实测置信度达到 0.9

2. 2. SQL Agent 节点 ( sql_agent_node )

   - 智能业务模块路由系统
   - 根据销量关键词（销量、销售、上险、品牌、车型等）自动路由
   - 支持销量查询、数据描述、通用查询三种模块

3. 3. 模块执行节点

   - 通过 module_executor 加载和执行销量查询模块
   - 自动加载模块配置 analysis_config.json
   - 返回结构化结果包含数据、分析、洞察和可视化配置

4. 4. 响应生成节点 ( response_generation_node )

   - 根据意图类型格式化最终响应
   - 对于 query_only 直接返回 SQL 查询结果
   - 支持错误处理和备用响应
...
1. 1. 车型精确查询
   ...
   - 智己 LS6 2024 年销量：33,719 辆（实测成功）
   - 智己 L6 vs 智己 LS6 对比：总计 52,406 辆（实测成功）

2. 2. GLM 智能参数提取

   - 品牌识别：智己、Tesla、比亚迪等
   - 车型识别：智己 LS6、智己 L6、Model Y 等
   - 时间解析：2024 年自动转换为 2024-01-01 到 2024-12-31
   - Token 消耗：平均 500-1100 tokens/查询

3. 3. 数据处理能力

   - 千万级数据支持：10,224,552 条记录
   - 查询响应时间：<1 秒
   - 0 销量记录自动过滤
   - AI 洞察自动生成

4. 4. 6 种查询模板
   - 品牌销量、车型销量、时间趋势、地区销量、燃料类型、综合销量
   - Jinja2 模板引擎支持
   - 自动模板选择
     🔧 技术架构优势：

- LangGraph 工作流 ：替代了原 Walker 架构，提供更灵活的节点编排
- 智能路由系统 ：根据关键词自动选择合适的业务模块
- GLM 集成 ：智能参数提取，正则表达式备用方案
- Gradio 界面 ：提供友好的 Web 交互界面
- 模块化设计 ：支持扩展新的查询类型和业务模块
  系统已成功实现从自然语言查询到结构化数据分析的完整工作流，具备强大的实时查询能力和智能分析功能。
